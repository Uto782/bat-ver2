...
export class BleClient {
  ...
  private cfg: BleConfig | null = null;
  ...
  public async connect(cfg: BleConfig): Promise<void> {
    if (!this.isSupported()) {
      throw new Error("Web Bluetoothが利用できません（ChromeまたはEdge推奨）");
    }
    ...
    this.characteristic = await service.getCharacteristic(cfg.characterUuid ?? cfg.characteristicUuid);
